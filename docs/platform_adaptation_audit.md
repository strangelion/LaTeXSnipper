...
| fixed | Startup Python resolver | `src/bootstrap/deps_python_runtime.py`, `src/runtime/python_runtime_resolver.py`, `src/runtime/main_bootstrap.py`, `LaTeXSnipper.spec`, `scripts/build_github_release_installer.ps1` | `ensure_full_python_or_prompt()` had a non-frozen fallback that only searched a few Windows executable names, and packaged Windows builds still carried Python installer discovery/packaging hooks. | Linux/macOS source-mode or non-packaged startup could fail to discover system `python3`; Windows custom dependency directories depended on a bundled installer path that increased installer size and diverged from the other platforms. | All platforms now share `find_system_python3()` for supported system Python `>=3.10,<3.13`, preferring 3.11. Windows packaged builds still use the bundled `deps/python311` template by default, but custom/missing dependency directories create a venv from system Python after user confirmation. The Python installer is no longer bundled or invoked by runtime code. |
