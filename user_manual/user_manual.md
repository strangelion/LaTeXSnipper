...
| 可设置快捷键 | `Ctrl+字母`、`Ctrl+Shift+字母` | 同 Windows | 同 Windows |
| 默认快捷键 | `Ctrl+F` | `Ctrl+F` | `Ctrl+F` |
...
| 依赖环境 | GitHub 安装包内置规范化依赖环境；Store 包内置 CPU 运行时和模型 | 运行时在 `~/.latexsnipper/deps/python311` 创建依赖环境，需要系统 Python 3.10+ | 同 Linux |
| 安装包 | Inno 安装包 / Store MSIX；GitHub Release 优先发布签名安装包，签名不可用时发布同名未签名回退包 | Debian/Ubuntu `.deb` | `.dmg` / `.app.zip` |

当前快捷键设置入口只接受 `Ctrl+字母` 或 `Ctrl+Shift+字母`，因此默认快捷键和用户可设置快捷键都落在三端共同支持范围内。若 Linux Wayland 环境下快捷键或截图无响应，通常是桌面环境权限或全局快捷键策略限制，优先参考下方 Wayland 说明。macOS 使用 Carbon 注册全局快捷键，通常不需要开启"辅助功能"权限；截图失败时应优先检查"屏幕录制"权限。
